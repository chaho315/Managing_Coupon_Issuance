...
class AsyncCouponIssueServiceV2Test extends TestConfig {
    ...
    void issue_2(){
        ...
        IntStream.range(0, coupon.getTotalQuantity()).forEach(ids -> {
            redisTemplate.opsForSet().add(getIssueRequestKey(coupon.getId()), String.valueOf(ids));
        });

        CouponIssueException exception = Assertions.assertThrows(CouponIssueException.class, () -> {
            sut.issue(coupon.getId(), userId);
        });
        Assertions.assertEquals(ErrorCode.INVAILD_COUPON_ISSUE_QUANTITY, exception.getErrorCode());
    }
    ...
    void issue_4(){
        ...
        Assertions.assertEquals(ErrorCode.INVAILD_COUPON_ISSUE_DATE, exception.getErrorCode());
    }

    @Test
    @DisplayName("쿠폰 발급 - 발급을 기록한다.")
    void issue_5(){
        long userId = 1;
        Coupon coupon = Coupon.builder()
                .couponType(CouponType.FIRST_COME_FIRST_SERVED)
    ...
    @Test
    @DisplayName("쿠폰 발급 - 발급을 요청이 성공하면 쿠폰 발급 큐에 적재된다.")
    void issue_6() throws JsonProcessingException {
        ...
        Assertions.assertEquals(saveIssueRequest,new ObjectMapper().writeValueAsString(request));
