...
import static org.example.couponcore.exception.ErrorCode.INVAILD_COUPON_ISSUE_DATE;
import static org.example.couponcore.exception.ErrorCode.INVAILD_COUPON_ISSUE_QUANTITY;
...
class CouponTest {
    ...
    @Test
    @DisplayName("발급 기간에 해당 되면 true를 반환한다")
    ...
    void issue_2(){
        ...
        Assertions.assertEquals(INVAILD_COUPON_ISSUE_QUANTITY, exception.getErrorCode());

    }

    @Test
    @DisplayName("발급 기간이 아니면 예외를 반환하다.")
    void issue_3(){
        //given
        Coupon coupon = Coupon.builder()
                .totalQuantity(100)
        ...
        //when&then
        CouponIssueException exception = Assertions.assertThrows(CouponIssueException.class, coupon::issue);
        Assertions.assertEquals(INVAILD_COUPON_ISSUE_DATE, exception.getErrorCode());
    }

    @Test
    @DisplayName("발급기간이 종료되면 true를 반환한다")
